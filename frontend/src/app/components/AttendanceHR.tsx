    ...
    <div className="w-full pb-10 fade-in space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        ...
        <div className="flex gap-2">
          ...
          <button className="px-4 py-2.5 bg-blue-600 text-white rounded-xl text-sm font-semibold hover:bg-blue-700 transition flex items-center gap-2">
            Export Report
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-blue-600 rounded-3xl p-6 text-white shadow-md relative overflow-hidden flex flex-col justify-between h-44">
        ...
        <div className="lg:col-span-2 grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            { label: "Pending Requests", value: data?.pendingRequests || 0, icon: Clock, color: "text-blue-600", bg: "bg-blue-50" },
        ...
        <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden flex flex-col h-[500px]">
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-4">
              ...
              <div key={req.id} className="p-4 bg-slate-50 border border-slate-100 rounded-2xl">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-500 truncate">{req.leave_type} • {req.start_date} to {req.end_date}</p>
                  </div>
                </div>
            ...
            {(!data?.latestLeaveRequests || data.latestLeaveRequests.length === 0) && (
              <p className="text-center text-slate-500 text-sm py-10">No pending leave requests.</p>
            )}
          </div>
        </div>
                    ...
                    <tr key={i} className="hover:bg-slate-50/50 transition">
                      ...
                      <td className="px-5 py-3">
                        <div className="flex items-center gap-1.5 text-xs font-medium text-slate-600">
                          {isRemote ? <MapPin size={14} className="text-indigo-500" /> : <Building size={14} className="text-blue-500" />}
