...
import {
  Zap, BrainCircuit, Activity, BarChart3, ArrowRight,
  CheckCircle, ChevronRight, Shield, TrendingDown, Users,
} from "lucide-react";
                  ...
                  <div className="flex items-end gap-2 h-16">
                    {[40, 55, 38, 62, 48, 72, 58, 80, 65, 88, 70, 95].map((h, i) => (
                      <div key={i} className="flex-1 rounded-sm"
                        style={{ height: `${h}%`, background: i >= 9 ? "#2563eb" : "#bfdbfe" }} />
                    ))}
                  </div>
                </div>
              </div>
        ...
        <div className="max-w-6xl mx-auto px-6 py-12">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
            {STATS.map(({ value, label }) => (
              <div key={label}>
                <div className="text-[#0f1f3d]" style={{ fontSize: "2rem", fontWeight: 800, letterSpacing: "-0.02em" }}>{value}</div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {FEATURES.map(({ icon: Icon, title, desc, accent, border }) => (
              <div key={title}
                ...
                <div>
                  <h3 className="text-[#0f1f3d] mb-2" style={{ fontSize: "1rem", fontWeight: 700 }}>{title}</h3>
                  <p className="text-gray-500" style={{ fontSize: "0.875rem", lineHeight: 1.7 }}>{desc}</p>
                </div>
        ...
        <div className="max-w-6xl mx-auto">
          <div className="grid md:grid-cols-2 gap-16 items-center">
            ...
            <div className="relative flex flex-col gap-4">
              {[
                { name: "Sarah Mitchell",   role: "Senior Engineer · Engineering", risk: "87%", riskColor: "text-red-600 bg-red-50",     action: "Schedule immediate 1:1 with HR",    avatar: "SM", avatarBg: "from-violet-400 to-purple-600" },
                { name: "Lucas Fernández",  role: "Data Analyst · Analytics",      risk: "79%", riskColor: "text-orange-600 bg-orange-50",action: "Offer lateral growth opportunity",  avatar: "LF", avatarBg: "from-rose-400 to-pink-600"     },
                { name: "Aisha Patel",      role: "HR Coordinator · HR",            risk: "22%", riskColor: "text-emerald-600 bg-emerald-50", action: "Continue regular check-ins",    avatar: "AP", avatarBg: "from-indigo-400 to-blue-600"   },
              ].map(({ name, role, risk, riskColor, action, avatar, avatarBg }, i) => (
                <div key={name} className="bg-white border border-gray-100 rounded-2xl p-5 flex items-center gap-4 shadow-sm"
                  ...
                  <div className={`shrink-0 px-3 py-1.5 rounded-xl ${riskColor}`} style={{ fontSize: "0.85rem", fontWeight: 800 }}>{risk}</div>
                </div>
              ))}
        ...
        <div className="max-w-6xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-5">
            {["Privacy", "Terms", "Support"].map((item) => (
              <a key={item} href="#" className="text-gray-400 hover:text-gray-600 transition-colors" style={{ fontSize: "0.78rem" }}>{item}</a>
            ))}
          </div>
        </div>
      </footer>
